...
export default function WelcomePage() {
  ...
  const yCity = useTransform(scrollY, [0, 200], [0, -150]);
  const yhero = useTransform(scrollY, [0, 200], [0, -200]);
  const opacityCityHero = useTransform(scrollY, [0, 300, 500], [1, 1, 0]);
  const scaleText = useTransform(scrollY, [0, 300], [1, 1.5]);
  const yText = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.4, 0]);
  ...
  return (
    <>
      <header id="welcome-header">
        ...
        />
        <motion.img
          style={{ y: yhero, opacity: opacityCityHero }}
          src={heroImg}
